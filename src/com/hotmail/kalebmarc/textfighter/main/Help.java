    ...
    private static void info_weapons() {
        while (true) {
            ...
            for (int i = 0; i < WeaponInventory.arrayWeapon.size(); i++) {
                Ui.println((i + 1) + ") " + WeaponInventory.arrayWeapon.get(i).getName());
            }
            Ui.println((WeaponInventory.arrayWeapon.size() + 1) + ") Back");
            Ui.println("------------------------------------------------------------");

            int menuItem = Ui.getValidInt();

            try {
                WeaponInventory.arrayWeapon.get(menuItem - 1).viewAbout();
            } catch (Exception e) {
                if (menuItem == (WeaponInventory.arrayWeapon.size() + 1)) return;
                Ui.println(menuItem + " is not an option.");
                Ui.pause();
            }
        }
